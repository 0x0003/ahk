...
CreateBitmap() {
  ...
  NumPut( 19778, BMP, 0, "UShort" )
  ; bfSize - specifies the size of the file in bytes.
  NumPut(    62, BMP, 2, "UShort" )
  ; reserved
  NumPut(     0, BMP, 6, "UInt"   )
  ; bfOffBits - specifies the offset from the beginning of the file to the bitmap data.
  NumPut(    54, BMP,10, "UInt"   )
  ; biSize - 40 specifies the size of the BITMAPINFOHEADER structure, in bytes.
  NumPut(    40, BMP,14, "UInt"   )
  ; biWidth - specifies the width of the image, in pixels.
  NumPut(     1, BMP,18, "UInt"   )
  ; biHeight - specifies the height of the image, in pixels.
  NumPut(     1, BMP,22, "UInt"   )
  ; biPlanes - specifies the number of planes of the target device, must be set to zero.
  NumPut(     1, BMP,26, "UShort" )
  ; biBitCount - specifies the number of bits per pixel.
  NumPut(    24, BMP,28, "UShort" )
  ; biCompression - Specifies the type of compression, usually set to zero (no compression).
  NumPut(     0, BMP,30, "UInt"   )
  ; biSizeImage - specifies the size of the image data, in bytes.
  NumPut(     8, BMP,34, "UInt"   )
  ; biXPelsPerMeter - specifies the the horizontal pixels per meter on the designated target device, usually set to zero.
  NumPut(     0, BMP,38, "UInt"   )
  ; biYPelsPerMeter - specifies the the horizontal pixels per meter on the designated target device, usually set to zero.
  NumPut(     0, BMP,42, "UInt"   )
  ; biClrUsed - specifies the number of colors used in the bitmap, if set to zero the number of colors is calculated using the biBitCount member.
  NumPut(     0, BMP,46, "UInt"   )
  ; biClrImportant - specifies the number of color that are 'important' for the bitmap, if set to zero, all colors are important
  NumPut(     0, BMP,50, "UInt"   )

  ; RGBQUAD - Data area
  NumPut( ColorGrid, BMP, 54, "UInt" ) ; Pixel Color - Black in BGR

  ; Open the file for writing rather than reading.
  GENERIC_WRITE = 0x40000000
  ; Create new file (overwriting any existing file).
  CREATE_ALWAYS = 2
...
CreateGrid() {
  ...
  Gui, +Owner +AlwaysOnTop -Resize -SysMenu -MinimizeBox -MaximizeBox -Disabled -Caption -Border -ToolWindow
  loop, % V.MaxIndex() {
    ; to convert the position to positive
    X := GetX(A_Index) - ScreenX
    ; to convert the position to positive
    Y := GetY0(A_Index) - ScreenY
  ...
  loop, % H.MaxIndex() {
    ; to convert the position to positive
    Y := GetY(A_Index) - ScreenY
    ; to convert the position to positive
    X := GetX0(A_Index) - ScreenX
